...
export default function SyncCustom() {
    ...
    const parseLocal = async (file: File): Promise<[boolean, any[] | null]> => {
        ...
        message.error('File parse exception');
        return [false, null];
    };

    const handleOk = () => {
        formRef.current?.form?.validateFields().then(async (vals: Record<string, any>) => {
            if ('new' === opInfo.opType) {
                if ('local' !== vals.protocol) {
                    delete vals.file;
    ...
    useInit(async () => {
        const filePath = await path.join(await chatRoot(), 'chat_app.log');
        setLogPath(filePath);
    });
