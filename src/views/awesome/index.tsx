...
export default function Awesome() {
    ...
    const handleOk = () => {
        formRef.current?.form?.validateFields().then(async (val: Record<string, any>) => {
            let idx = opData.findIndex((i) => i.url === val.url);
            if (val.url === opInfo?.opRecord?.url) {
                idx = -1;
            }
            if (idx === -1) {
                if (opInfo.opType === 'new') {
                    const data = opAdd(val);
                    await updateJson(data);
                    opInit(data);
                    message.success('Data added successfully');
                }
                if (opInfo.opType === 'edit') {
                    const data = opReplace(opInfo?.opRecord?.[opSafeKey], val);
                    await updateJson(data);
