...
pub fn init(app: &mut App) -> Result<(), Box<dyn std::error::Error>> {
  ...
  tauri::async_runtime::spawn(async move {
    app::script::init_script(handle)
      .await
      .unwrap_or_else(|err| {
        error!("init script file occur error:{}", err);
      });
  });
  ...
  if let Some(v) = app_conf.clone().global_shortcut {
    info!("global shoart cut: {}", v);
    match v.parse::<Accelerator>() {
      Ok(_) => {
        info!("global shortcut register....");
        let handle = app.app_handle();
        let mut shortcut = app.global_shortcut_manager();
        shortcut
          .register(&v, move || {
            if let Some(win) = handle.get_window("core") {
          ...
          .unwrap_or_else(|err| {
            error!("global shortcut register occur error:{}", err);
          });
      }
      Err(err) => {
        error!("global shortcut parse error:{}", err);
      }
    }
  } else {
    info!("global shortcut unregister....");
  };
  ...
  if app_conf.hide_dock_icon {
    ...
    tauri::async_runtime::spawn(async move {
      let url_clone = &url;
      let mut main_win = WindowBuilder::new(&app, "core", WindowUrl::App(url_clone.into()))
        .title("ChatApp")
        .resizable(true)
        .fullscreen(false)
      ...
      if let (Err(err)) = main_win.build() {
        error!("core window build error:{}", err);
      }
    });
  }

  let auto_update = app_conf.get_auto_update();
  if "disable" != auto_update {
    info!("check update....");
    let app = app.handle();
