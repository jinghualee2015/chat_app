...
pub async fn init_script(app: tauri::AppHandle) -> anyhow::Result<(), reqwest::Error> {
  let body = reqwest::get(format!("{}{}", SCRIPTS_DIR, MANIFEST_NAME))
    .await?
    .text()
    .await?;

  if exitst_scripts(MANIFEST_NAME.into()) {
    let compare = compare_nested_json_objects(
      &fs::read_to_string(app_root().join("scripts").join(MANIFEST_NAME)).unwrap(),
      &body,
    );
    if !compare {
      info!("update script manifest file...");
      create_chatgpt_scripts(MANIFEST_NAME.into(), body);
...
pub fn parse_script(name: String) -> Option<serde_json::Value> {
  ...
  info!("script file={:?}", script_file.to_str());
  let code = match fs::read_to_string(&script_file) {
    Ok(contnet) => contnet,
    Err(_) => {
      error!("parse script error:{}", script_file.display());
      return None;
    }
  };
...
pub fn exitst_scripts(file: String) -> bool {
...
pub fn create_chatgpt_scripts(file: String, body: String) {
  ...
  match create_file(&script_file) {
    ...
    Err(err) => {
      error!("create_file, {}:{}", script_file.display(), err);
    }
  }
}

pub fn compare_nested_json_objects(json1: &str, json2: &str) -> bool {
