...
pub fn init() -> Menu {
  ...
  let app_menu = Submenu::new(
    ...
    Menu::with_items([
      ...
      CustomMenuItem::new("about", "About ChatApp").into(),
      CustomMenuItem::new("check_update", "Check for Update").into(),
  ...
  let popup_search = CustomMenuItem::new("popup_search", "Popup Search");
  ...
  let preferences_menu = Submenu::new(
    ...
    Menu::with_items([
      ...
      CustomMenuItem::new("go_conf", "Go to Config")
        ...
        .into(),
      CustomMenuItem::new("restart", "Restart ChatApp")
        .accelerator("CmdOrCtrl+Shift+R")
        .into(),
      CustomMenuItem::new("clear_conf", "Clear Config").into(),
      MenuItem::Separator.into(),
      CustomMenuItem::new("chatapp_sponsors", "ChatApp Sponsors").into(),
      MenuItem::Separator.into(),
  ...
  let window_menu = Submenu::new(
    "Window",
    Menu::new()
      .add_item(CustomMenuItem::new("app_website", "User Guide"))
  ...
  let help_menu = Submenu::new(
    "Help",
    Menu::new()
      .add_item(CustomMenuItem::new("chatapp_log", "ChatApp Log"))
...
pub fn menu_handler(event: WindowMenuEvent<tauri::Wry>) {
  ...
  match menu_id {
    ...
    "check_update" => utils::run_check_update(app, false, None),
    "control_center" => {
      log::info!("click control center menu....");
      window::cmd::control_window(app, "control".into())
    }
    ...
    "app_website" => window::cmd::wa_window(
      app,
      "app_website".into(),
      "ChatApp User's Guide".into(),
      conf::APP_WEBSITE.into(),
      None,
    ),
    "nofwl" => open(&app, conf::NOFWL_APP),
    "chatapp_sponsors" => window::cmd::wa_window(
      app,
      "app_sponsors".into(),
      "Sponsors Page".into(),
    ...
    "sync_prompts" => tauri::api::dialog::ask(
      app.get_window("core").as_ref(),
      "Sync Prompts",
      "Data sync will enable all prompts, are sure you want to sync?",
      move |is_restart| {
        if is_restart {
          app
            .get_window("core")
    ...
    "zoom_0" => win.eval("window._zoom0 && window._zoom0()").unwrap(),
    "zoom_out" => win.eval("window._zoomOut && window._zoomOut()").unwrap(),
    "zoom_in" => win.eval("window._zoomIn && window._zoomIn()").unwrap(),
    "reload" => win.eval("window.localtion.reload()").unwrap(),
    ...
    "scrop_bottom" => win
      ...
      .unwrap(),
    "chatapp_log" => utils::open_file(utils::app_root().join("chat_app.log")),
    "update_log" => open(&app, conf::UPDATE_LOG_URL),
    "reprt_bug" => open(&app, conf::ISSUES_URL),
    "dev_tools" => {
      win.open_devtools();
      win.close_devtools();
    }
