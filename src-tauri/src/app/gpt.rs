...
pub fn cmd_list() -> Vec<PromptRecord> {
  ...
  for entry in WalkDir::new(utils::app_root().join("cache_prompt"))
    .into_iter()
    .filter_map(|e| e.ok())
  {
...
pub fn download_list(pathname: &str, dir: &str, filename: Option<String>, id: Option<String>) {
  ...
  utils::vec_to_hasmap(data.0.into_iter(), "id", &mut idmap);
  ...
  if let Some(v) = res {
    if let Some(data) = parse_prompt(v) {
      ...
      let chatgpt_prompts = utils::app_root()
        .join("cache_prompts")
        .join("chat_app_prompts.json");

      if !utils::exists(&prompts) {
        fs::write(
          &prompts,
          serde_json::json!({
            "name": "ChatApp Prompts",
...
pub async fn sync_user_prompts(url: String, data_type: String) -> Option<Vec<PromptRecord>> {
  ...
  let res = utils::get_data(&url, None).await.unwrap_or_else(|err| {
    error!("chat_app_http: {}", err);
    None
  });

  if let Some(v) = res {
    let data: Option<Vec<PromptBaseRecord>> = if data_type == "csv" {
      info!("chat_app_http_csv_parse");
      parse_prompt(v)
    } else if data_type == "json" {
      info!("chat_app_http_json_parse");
      match serde_json::from_str::<Vec<PromptBaseRecord>>(&v) {
        Ok(parsed) => Some(parsed),
        Err(err) => {
          error!("chat_app_http_json_parse: {}", err);
          None
        }
      }
    } else {
      error!("chat_app_http_unknown_type");
      None
    };
