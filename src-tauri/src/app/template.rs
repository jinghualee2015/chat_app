      ...
      for (filename, dest) in files {
        if !filename.exists() {
          match create_dir(&filename) {
            Ok(_) => {
              if let Err(e) = write_file_content(&filename, &dest) {
            ...
            Err(err) => {
              error!("create file, {}: {}", filename.display(), err);
            }
          }
        }
      }
...
fn create_dir<P: AsRef<Path>>(file_name: P) -> Result<()> {
...
fn write_file_content<P: AsRef<Path>>(filename: P, data: &[u8]) -> Result<()> {
...
mod tests {
  ...
  #[test]
  pub fn test_new() {
    let path = Path::new("/Users/jinghuali/tauri_app");
