  ...
  tauri::async_runtime::spawn(async move {
    if let Some(win) = handle.get_window("sponsor") {
      ...
      WindowBuilder::new(
        &handle,
        "sponsor",
        WindowUrl::App("sponsor-win.html".into()),
      )
      .title("Sponsor")
      .resizable(true)
      .fullscreen(false)
...
pub mod cmd {
  ...
  pub fn dalle2_search_window(app: tauri::AppHandle, query: String) {
    dalle2_window(
      &app.app_handle(),
      Some(query),
      Some("ChatApp & DALL·E 2".to_string()),
      None,
    );
  }
  ...
  pub fn control_window(handle: tauri::AppHandle, win_type: String) {
    info!("control_window command execute, win_type:{}", win_type);
    tauri::async_runtime::spawn(async move {
  ...
  pub fn wa_window(
    ...
    if win.is_none() {
      ...
      v.eval("window.localtion.reload()").unwrap();
      v.set_focus().unwrap();
    }
  }
