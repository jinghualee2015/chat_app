...
pub fn app_root() -> PathBuf {
  let path = tauri::api::path::home_dir().unwrap().join(".chat_app");
  info!("get current app root path:{:?}", path.clone());
  path
}

/// get tauri.conf.json file info and generate config json content
pub fn get_tauri_conf() -> Option<Config> {
  let confg_file = include_str!("../tauri.conf.json");
  let config = serde_json::from_str(confg_file).expect("failed to parse tauri.conf.json");
  Some(config)
}
...
pub fn create_file<P: AsRef<Path>>(file_name: P) -> Result<()> {
  let filename = file_name.as_ref();
...
pub fn open_file(path: PathBuf) {
  let path_name = convert_path(path.to_str().unwrap());
  info!("open file: {}", path_name);
...
pub fn clear_conf(app: &tauri::AppHandle) {
  ...
  let msg = format!(
        ...
        Note: The application will exit automatically after the configuration cleanup!",root.to_string_lossy());
...
pub async fn get_data(
  ...
  if is_ok {
    ...
    error!("chat_app_http: {}", body);
    if let Some(v) = app {
      tauri::api::dialog::message(v.get_window("core").as_ref(), "ChatApp HTTP", body);
    }
    Ok(None)
  }
}
